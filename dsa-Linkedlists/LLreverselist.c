...
void reverseList() {
    ...
    // Traversing the linked list and reversing the nodes
    while(current != NULL) {
        next = current->nextptr; // Storing the next node
        current->nextptr = prev; // Reversing the link
        prev = current;          // Moving prev to the current node
        current = next;          // Moving current to the next node
    }

    // Updating the starting node to the new first node
    stnode = prev;
}