...
int main() {
    ...
    printf("the number of nodes in the single linked list is:%d\n",countNodes());
...
int countNodes() {
    ...
    if(stnode == NULL) {
        ...
        // Traversing the linked list and counting the number of nodes
        while(tmp != NULL) {
            count++; // Incrementing the count for each node
            tmp = tmp->nextptr; // Moving to the next node in the list
        }
        return count;
    }
